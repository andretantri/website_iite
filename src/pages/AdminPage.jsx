        ...
        <main className="flex-1 overflow-y-auto p-4 lg:p-8 space-y-6">
          ...
          {activeItem.id === 'news' && (
            <div className="space-y-6">
              ...
              {/* News Article Modal (Filament Style 2-Column Layout) */}
              {isArticleFormOpen && (
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/60 backdrop-blur-sm p-4 overflow-y-auto">
                  <div className="w-full max-w-5xl max-h-[92vh] overflow-y-auto rounded-3xl border border-slate-200 bg-white p-6 sm:p-8 shadow-2xl space-y-6">
                    {/* Header */}
                    <div className="flex items-center justify-between border-b border-slate-200 pb-4">
                      <div>
                        <h3 className="font-extrabold text-slate-900 text-lg sm:text-xl">
                          {editingArticleIndex !== null ? 'Edit News Article' : 'Create News Article'}
                        </h3>
                        <p className="text-xs text-slate-500 mt-0.5">Kelola konten publikasi berita, media, dan optimasi SEO.</p>
                      </div>
                      <button
                        type="button"
                    ...
                    <form onSubmit={handleSaveArticle} className="space-y-6">
                      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
                        {/* LEFT COLUMN: Main Image, Is Published, SEO Fields */}
                        <div className="lg:col-span-6 space-y-6">
                          {/* Main Image */}
                          <div className="rounded-2xl border border-slate-200 bg-slate-50/50 p-5 space-y-3">
                            <label className="text-xs font-bold uppercase tracking-wider text-slate-700 block">Main Image</label>
                            <div className="border-2 border-dashed border-slate-300 rounded-xl p-4 text-center bg-white hover:border-indigo-400 transition relative">
                              {articleForm.image ? (
                                <div className="space-y-3">
                                  <div className="aspect-video w-full rounded-lg overflow-hidden bg-slate-100 border border-slate-200 relative group">
                                    <img src={articleForm.image} alt="Main Preview" className="h-full w-full object-cover" />
                                    <button
                                      type="button"
                                      onClick={() => setArticleForm(prev => ({ ...prev, image: '' }))}
                                      className="absolute top-2 right-2 p-1.5 rounded-lg bg-rose-600 text-white shadow hover:bg-rose-700 transition text-xs"
                                    >
                                ...
                                <div className="py-6 space-y-2">
                                  ...
                                  <p className="text-xs text-slate-600 font-medium">
                                    Drag & Drop your files or <span className="text-indigo-600 font-bold hover:underline cursor-pointer">Browse</span>
                                  </p>
                                  <input
                                    type="file"
                                    accept="image/*"
                          ...
                          <div className="flex items-center justify-between rounded-2xl border border-slate-200 bg-white p-4">
                            <div>
                              <span className="text-xs font-bold text-slate-800 block">Is published</span>
                              <span className="text-[11px] text-slate-500">Aktifkan untuk mempublikasikan artikel secara publik.</span>
                            </div>
                            <button
                              type="button"
                          ...
                          <div className="space-y-1.5">
                            <label className="text-xs font-bold text-slate-700 block">Meta Title (SEO)</label>
                            <input
                              type="text"
                            ...
                            />
                            <p className="text-[11px] text-slate-400">Judul di Google/Tab Browser. Maks 60 karakter. Otomatis terisi dari judul berita.</p>
                          </div>

                          {/* Meta Description (SEO) */}
                          <div className="space-y-1.5">
                            <label className="text-xs font-bold text-slate-700 block">Meta Description (SEO)</label>
                            <textarea
                              rows={3}
                            ...
                            />
                            <p className="text-[11px] text-slate-400">Deskripsi di Google, idealnya 120-160 karakter.</p>
                          </div>

                          {/* Meta Keywords */}
                          <div className="space-y-1.5">
                            <label className="text-xs font-bold text-slate-700 block">Meta Keywords</label>
                            <input
                              type="text"
                              value={articleForm.metaKeywords}
                              onChange={(e) => setArticleForm(prev => ({ ...prev, metaKeywords: e.target.value }))}
                              className="w-full rounded-xl border border-slate-300 bg-white px-3.5 py-2 text-xs text-slate-800 focus:border-indigo-500 focus:ring-2 focus:ring-indigo-100"
                              placeholder="kata kunci 1, kata kunci 2, kata kunci 3"
                            />
                            <p className="text-[11px] text-slate-400">Kata kunci dipisah koma. Contoh: politeknik, berita, akademik.</p>
                          </div>

                          {/* Tags */}
                          <div className="space-y-1.5">
                            <label className="text-xs font-bold text-slate-700 block">Tags</label>
                            <input
                              type="text"
                            ...
                            />
                            <p className="text-[11px] text-slate-400">Tag untuk berita terkait. Dipisah koma. Contoh: wisuda, akademik, kegiatan.</p>
                          </div>
                        </div>

                        {/* RIGHT COLUMN: Content, Additional Images, Date */}
                        <div className="lg:col-span-6 space-y-6">
                          {/* Additional Images Section */}
                          <div className="rounded-2xl border border-slate-200 bg-slate-50/50 p-5 space-y-3">
                            <div className="flex items-center justify-between">
                              <label className="text-xs font-bold uppercase tracking-wider text-slate-700 block">Additional Images</label>
                              <label className="inline-flex items-center gap-1 bg-indigo-600 text-white rounded-lg px-3 py-1.5 cursor-pointer text-xs font-bold hover:bg-indigo-700 transition">
                                <Plus className="h-3.5 w-3.5" /> Add to additional images
                                <input
                                  type="file"
                                  accept="image/*"
                                  className="hidden"
                            ...
                            {articleForm.additionalImages && articleForm.additionalImages.length > 0 ? (
                              <div className="grid grid-cols-3 gap-3">
                                {articleForm.additionalImages.map((imgUrl, i) => (
                                  <div key={i} className="aspect-square rounded-xl overflow-hidden bg-slate-200 border border-slate-300 relative group">
                                    <img src={imgUrl} alt={`Addl ${i + 1}`} className="h-full w-full object-cover" />
                                    <button
                                      type="button"
                                      onClick={() => {
                                        setArticleForm(prev => ({
                                      ...
                                      className="absolute top-1.5 right-1.5 p-1 rounded-md bg-rose-600 text-white opacity-0 group-hover:opacity-100 transition text-xs shadow"
                                    >
                                      <Trash2 className="h-3.5 w-3.5" />
                                    </button>
                                  </div>
                              ...
                              <p className="text-xs text-slate-400 italic text-center py-4">Belum ada gambar tambahan.</p>
                            )}
                          </div>

                          {/* Tanggal Publish */}
                          <div className="space-y-1.5">
                            <label className="text-xs font-bold text-slate-700 block">Tanggal Publish</label>
                            <input
                              type="text"
                              required
                              ...
                              className="w-full rounded-xl border border-slate-300 bg-white px-3.5 py-2 text-xs text-slate-800 focus:border-indigo-500 focus:ring-2 focus:ring-indigo-100"
                              placeholder="19 Jul 2026, 22:24 atau 20 Maret 2026"
                            />
                            <p className="text-[11px] text-slate-400">Tanggal dan waktu berita ini diterbitkan. Kosongkan untuk menggunakan tanggal saat ini.</p>
                          </div>

                          {/* Judul Artikel */}
                          <div className="space-y-1.5">
                            <label className="text-xs font-bold uppercase tracking-wider text-slate-700 block">Judul Artikel Berita</label>
                            <input
                              type="text"
                              required
                              value={articleForm.title}
                              onChange={handleTitleChange}
                              className="w-full rounded-xl border border-slate-300 bg-white px-3.5 py-2.5 text-sm text-slate-800 focus:border-indigo-500 focus:ring-2 focus:ring-indigo-100 font-semibold"
                          ...
                          {/* Kategori */}
                          <div className="space-y-1.5">
                            <label className="text-xs font-bold uppercase tracking-wider text-slate-700 block">Kategori Berita</label>
                            <input
                              type="text"
                              required
                              ...
                              className="w-full rounded-xl border border-slate-300 bg-white px-3.5 py-2 text-xs text-slate-800 focus:border-indigo-500 focus:ring-2 focus:ring-indigo-100"
                              placeholder="Pengumuman, Pembicara, Wisuda, Kegiatan..."
                            />
                          </div>

                          {/* Ringkasan Singkat */}
                          <div className="space-y-1.5">
                            <label className="text-xs font-bold uppercase tracking-wider text-slate-700 block">Ringkasan Singkat (Summary)</label>
                            <textarea
                              rows={2}
                              required
                              ...
                              className="w-full rounded-xl border border-slate-300 bg-white px-3.5 py-2 text-xs text-slate-800 focus:border-indigo-500 focus:ring-2 focus:ring-indigo-100"
                              placeholder="Ringkasan singkat untuk kartu berita di beranda..."
                            />
                          </div>

                          {/* Isi Lengkap Artikel (Rich Text Editor) */}
                          <div className="space-y-1.5">
                            <label className="text-xs font-bold uppercase tracking-wider text-slate-700 block">Isi Lengkap Artikel (Rich Text)</label>
                            <RichTextEditor
                      ...
                      {/* Action Buttons Bar matching Filament footer */}
                      <div className="flex flex-wrap items-center justify-between border-t border-slate-200 pt-5 gap-3">
                        <button
                          type="button"
                          onClick={() => setIsArticleFormOpen(false)}
                        ...
                        <div className="flex items-center gap-3">
                          <button
                            ...
                            className="px-5 py-2.5 rounded-xl border border-indigo-600 text-indigo-600 text-xs font-bold hover:bg-indigo-50 transition"
                          >
                          ...
                          <button
                            type="submit"
                            className="px-6 py-2.5 rounded-xl bg-indigo-600 text-xs font-bold text-white hover:bg-indigo-700 shadow-md shadow-indigo-200 transition"
                          >
                            {editingArticleIndex !== null ? 'Save Changes' : 'Create'}
                          </button>
                        </div>
                      </div>
                    </form>
