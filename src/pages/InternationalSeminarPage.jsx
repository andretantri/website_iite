      ...
      {(accent) => (
        <>
              ...
              <div className="grid gap-8 lg:grid-cols-2 lg:items-center">
                ...
                <div className="order-1 lg:order-2 rounded-[32px] border border-white/10 bg-iite-dark/80 p-8 shadow-glass backdrop-blur-xl sm:p-12">
                  ...
                  <div className="mt-8 flex flex-wrap gap-4">
                    <a href="https://proceeding.iite.io/index.php/iitp/login" target="_blank" rel="noopener noreferrer" className={`inline-flex items-center gap-2 rounded-full px-6 py-3 text-sm font-semibold transition hover:-translate-y-0.5 ${accent.btn}`}>
                      {p.joinBtn}
              ...
              <div className="mx-auto max-w-7xl">
                ...
                <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-4">
                  {p.speakers.map((speaker, i) => (
                    <div key={i} className={`group rounded-[24px] border border-white/10 bg-white/[0.04] p-6 backdrop-blur-sm transition duration-500 hover:-translate-y-1 hover:border-iite-purple/30 hover:bg-white/[0.08]`}>
                      ...
                      <p className="mt-3 text-sm leading-7 text-slate-300">{speaker.role}</p>
                    </div>
                  ))}
                </div>
              </div>
          ...
          <section className="fade-up px-4 py-20 sm:px-6 lg:px-8">
            <div className="mx-auto max-w-7xl">
              <h2 className="mb-2 text-sm uppercase tracking-[0.26em] text-iite-purple">{p.categoriesTag}</h2>
                    ...
                    <div className="space-y-3 text-sm text-slate-300">
                      {p.payments.map((pay, i) => (
                        <div key={i} className="mb-2">
                          ...
                          <p><span className="text-slate-400">Account Name:</span> <span className="text-white font-medium">{pay.name}</span></p>
                        </div>
                      ))}
                    </div>
                  </div>
          ...
          <section className="fade-up px-4 py-20 sm:px-6 lg:px-8">
            <div className="mx-auto max-w-7xl">
              <h2 className="mb-2 text-sm uppercase tracking-[0.26em] text-iite-purple">{p.timelineTag}</h2>
              ...
              <div className="relative border-l-2 border-iite-purple/30 pl-8">
                {p.timeline.map((item, i) => (
                  <div key={i} className="relative mb-12 last:mb-0">
                    <div className="absolute -left-[41px] top-1 h-4 w-4 rounded-full border-2 border-iite-dark bg-gradient-to-br from-purple-400 to-fuchsia-600 shadow-lg shadow-purple-400/20" />
                    <div className="rounded-2xl border border-white/10 bg-white/[0.04] px-6 py-4 backdrop-blur-sm transition hover:border-iite-purple/30 hover:bg-white/[0.07]">
                      <p className="text-sm font-bold text-iite-purple">{item.date}</p>
          ...
          <section className="fade-up relative overflow-hidden">
            ...
            <div className="relative px-4 py-20 sm:px-6 lg:px-8">
              <div className="mx-auto max-w-7xl">
                <div className="grid gap-8 lg:grid-cols-2">
                  <div>
                    <h2 className="mb-2 text-sm uppercase tracking-[0.26em] text-iite-purple">{p.scheduleTag}</h2>
                    <h3 className="mb-8 text-3xl font-semibold text-white">{p.scheduleTitle}</h3>
                    <div className="grid gap-6">
                      {p.schedule.map((item, i) => (
                        <div key={i} className="flex items-start gap-4 rounded-2xl border border-white/10 bg-white/[0.04] px-6 py-4 backdrop-blur-sm">
                          <CalendarDays className="h-6 w-6 text-iite-purple flex-shrink-0" />
                          <div>
                            <p className="font-semibold text-white">{item.date} | {item.time}</p>
                            <p className="text-sm text-slate-300">{item.platform}</p>
                          </div>
                        </div>
                      ))}
                    </div>
                  ...
                  <div>
                    <h2 className="mb-2 text-sm uppercase tracking-[0.26em] text-iite-purple">{p.contactsTag}</h2>
                    ...
                    <div className="grid gap-6">
                      {p.contacts.map((contact, i) => (
                        <div key={i} className="flex items-center gap-4 rounded-2xl border border-white/10 bg-white/[0.04] px-6 py-4 backdrop-blur-sm">
                          <Phone className="h-5 w-5 text-iite-purple" />
                          <div>
                            <p className="font-semibold text-white">{contact.name}</p>
                            <a href={getWhatsAppUrl(contact.phone)} target="_blank" rel="noopener noreferrer" className="text-sm text-iite-purple hover:text-purple-300 hover:underline">
                              {contact.phone}
                            </a>
                          </div>
                        </div>
