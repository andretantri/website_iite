import { BookOpen, FileText, Search, CheckCircle2, ArrowRight } from 'lucide-react'
    ...
    <PageLayout theme={theme} accentColor="rose">
      {(accent) => (
        <>
          ...
          <section className="fade-up px-4 py-16 sm:px-6 lg:px-8">
            <div className="mx-auto max-w-7xl">
              <h2 className="mb-2 text-sm uppercase tracking-[0.26em] text-rose-400">{p.topicsTag}</h2>
              <h3 className="mb-8 text-3xl font-semibold text-white">{p.topicsTitle}</h3>
                ...
                {p.topics.map((topic, i) => (
                  <div key={i} className={`glass-card rounded-[28px] border border-white/10 p-6 shadow-glass transition duration-500 hover:-translate-y-1 ${accent.card}`}>
                    <div className={`mb-4 inline-flex h-12 w-12 items-center justify-center rounded-2xl bg-white/5 ${accent.icon}`}>
                      {i % 3 === 0 ? <BookOpen className="h-6 w-6" /> : i % 3 === 1 ? <Search className="h-6 w-6" /> : <FileText className="h-6 w-6" />}
                    </div>
                    <h4 className="text-lg font-semibold text-white">{topic.title}</h4>
                    <p className="mt-3 text-sm leading-7 text-slate-300">{topic.desc}</p>
                  </div>
                ))}
              </div>
            </div>
          ...
          <section className="fade-up px-4 py-16 sm:px-6 lg:px-8">
            <div className="mx-auto max-w-7xl">
              <div className="rounded-[32px] border border-white/10 bg-gradient-to-br from-rose-500/10 via-rose-500/5 to-transparent p-8 sm:p-12">
                <h2 className="mb-2 text-sm uppercase tracking-[0.26em] text-rose-400">{p.guidelinesTag}</h2>
                <h3 className="mb-8 text-3xl font-semibold text-white">{p.guidelinesTitle}</h3>
                <div className="grid gap-4 sm:grid-cols-2">
                  {p.guidelines.map((item, i) => (
